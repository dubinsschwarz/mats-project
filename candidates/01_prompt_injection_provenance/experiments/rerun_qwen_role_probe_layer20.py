    ...
    try:
        ...
        for start, prompt_ix in enumerate(active_prompts):
            ...
            if start % 100 == 0:
                ...
                log_peak_rss(f"activation_{start}")

        log_peak_rss("activation_complete")
        ...
        for layer in LAYERS:
            ...
            for split_name, (train_mask, test_mask) in split_masks.items():
                log_peak_rss(f"before_fit_layer_{layer}_{split_name}")
                ...
                progress = {
                    ...
                    "completed_splits": list(layer_results),
                    "last_completed_split": split_name,
                    "scientific_status": "partial_until_both_splits_complete",
                }
                (OUT / "layer20_fit_progress.json").write_text(
                    json.dumps(progress, indent=2) + "\n"
                )
                log_peak_rss(f"after_fit_layer_{layer}_{split_name}")
            ...
            progress = {
                "layer_zero_based": layer,
                "completed_splits": list(split_masks),
                "last_completed_split": "passage_grouped",
                "scientific_status": "complete",
                "validation_decision": layer_results["validation_decision"],
            }
            (OUT / "layer20_fit_progress.json").write_text(
                json.dumps(progress, indent=2) + "\n"
            )
        ...
        passed = [layer for layer in DECISION_LAYERS if results[str(layer)]["validation_decision"]["validated"]]
        if not passed:
            ...
            "layers_zero_based": DECISION_LAYERS,
